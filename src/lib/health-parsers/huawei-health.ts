 ...
 * JSON files in directories like motion/step/, sleep/, sport/, stress/, spo2/.
 */
export const huaweiHealthParser: HealthParser = {
  ...
  async parse(directory: ZipDirectory): Promise<ParseResult> {
    ...
    for (const file of directory.files) {
      ...
      if (name.includes("step")) { metric = "steps"; unit = "count"; }
      else if (name.includes("heartrate") || name.includes("heart_rate")) { metric = "heartRate"; unit = "bpm"; }
      else if (name.includes("sleep")) { metric = "sleepAnalysis"; unit = "min"; }
      else if (name.includes("sport") || name.includes("workout") || name.includes("exercise")) { metric = "workout"; unit = "min"; }
      else if (name.includes("stress")) { metric = "stress"; unit = "score"; }
      else if (name.includes("spo2") || name.includes("oxygen")) { metric = "bloodOxygen"; unit = "%"; }
      else if (name.includes("weight") || name.includes("body")) { metric = "weight"; unit = "kg"; }
      else if (name.includes("calorie")) { metric = "calories"; unit = "kcal"; }
      else if (name.includes("distance")) { metric = "distance"; unit = "km"; }

      if (!metric) continue;

      for (const item of data) {
        const rec = item as Record<string, unknown>;
        const startDate = parseDate(rec.startTime ?? rec.start_time ?? rec.time ?? rec.dateTime);
        const endDate = parseDate(rec.endTime ?? rec.end_time ?? rec.time ?? rec.dateTime) ?? startDate;
        ...
        records.push({
          metric,
          value: metric === "sleepAnalysis" && rec.duration ? parseFloat(String(rec.duration)) / 60000 : value,
          unit,
          startDate,
