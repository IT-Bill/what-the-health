...
async function main() {
  ...
  // --- Credit Transactions (demo history for elena) ---
  ...
  let balance = 2450; // elena's starting credits
  // Rebuild balance from initial (we set user.credits = 2450 which represents all past earning)
  // These transactions are recent ones on top of that balance minus the spending
  balance = 2450 - 109; // transactions below sum to +109 net, so start was 2450-109=2341 before these
  for (const tx of txns) {
    balance += tx.amount;
    await prisma.creditTransaction.create({
      data: { userId: user.id, action: tx.action, direction: tx.direction, amount: Math.abs(tx.amount), balance, note: tx.note },
    });
  }
  // Final balance should be 2450 (matches user.credits)
